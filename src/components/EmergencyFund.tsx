...
export function EmergencyFund({ onManage }: { onManage: () => void }) {
  const { emergencyFund } = useInvestmentsStore()
  const {
    totalCosts,
    emergencyFundTarget: target,
    emergencyFundRemaining: remaining,
    emergencyFundProgress: progress,
    emergencyFundMonthsToGoal: monthsToGoal,
  ...
  return (
    <Card
      ...
      headerExtra={
        target > 0 ? (
          // A reserva é patrimônio acumulado, não uma fatia do orçamento do mês:
          // o percentual aqui é do alvo em meses de custo, não da renda.
          <HeaderMetric
      ...
      <div className="space-y-4">
        ...
        {totalCosts > 0 ? (
          <>
            <div>
              <div className="mb-1.5 flex items-baseline justify-between text-xs">
                <span className="text-dark-text-muted">
                  Meta: {formatCurrency(target)} ({emergencyFund.targetMonths} meses de custos
                  fixos)
                </span>
            ...
            <p className="text-sm leading-relaxed text-dark-text-secondary">
              {remaining <= 0 ? (
                ...
                <>
                  Faltam <strong className="text-dark-text">{formatCurrency(remaining)}</strong>. No
                  ritmo atual de {formatCurrency(fixedIncomeMonthlyAllocation)}/mês em renda fixa,
                  você completa em{' '}
                  <strong className="text-dark-text">{formatMonths(monthsToGoal)}</strong>.
                </>
              ) : (
                <>
                  Faltam <strong className="text-dark-text">{formatCurrency(remaining)}</strong>.
                  Direcione parte do aporte para renda fixa para estimar o prazo.
                </>
              )}
            </p>
          </>
          ...
          <p className="text-sm text-dark-text-muted">
            Cadastre os custos fixos para calcular a meta — a reserva ideal cobre alguns meses do
            seu custo de vida.
          </p>
        )}
