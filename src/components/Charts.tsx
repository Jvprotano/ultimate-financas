...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ name: string; value: number; payload: { color?: string; fill?: string } }> }) {
...
function renderLegend(props: any) {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-2">
      {payload.map((entry) => (
        <span key={entry.value} className="flex items-center gap-1.5 text-xs text-dark-text-secondary">
...
export function Charts({ budgetAllocation, investmentAllocation, costsByCategory, availableForBudget }: Props) {
  if (availableForBudget <= 0) return null

  const budgetData = [
    { name: 'Necessidades', value: budgetAllocation.necessidades, color: BUDGET_COLORS[0] },
    { name: 'Desejos', value: budgetAllocation.desejos, color: BUDGET_COLORS[1] },
    { name: 'Investimentos', value: budgetAllocation.investimentos, color: BUDGET_COLORS[2] },
  ].filter((d) => d.value > 0)
  ...
  return (
    <Card title="Visao Geral" icon={<BarChart3 size={18} />} accentColor="bg-slate-600">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {/* Budget Split */}
        <div>
          <h3 className="text-sm font-semibold text-dark-text-secondary mb-3 text-center">Divisao do Orcamento</h3>
        ...
        {investmentData.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-dark-text-secondary mb-3 text-center">Diversificacao</h3>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie
                  data={investmentData}
        ...
        {costData.length > 0 && (
          <div className="md:col-span-2 xl:col-span-1">
            <h3 className="text-sm font-semibold text-dark-text-secondary mb-3 text-center">Custos por Categoria</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={costData} layout="vertical" margin={{ left: 10, right: 20, top: 5, bottom: 5 }}>
                ...
                <XAxis type="number" tickFormatter={(v: number) => formatCurrency(v)} tick={{ fontSize: 11, fill: '#94a3b8' }} />
